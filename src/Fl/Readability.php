...
namespace Aboustayyef\Fl;
use \DOMDocument;
use \RuntimeException;
use \Exception;
// source of code: https://github.com/feelinglucky/php-readability
...
class Readability {
    // Save determination result flag name - 保存判定结果的标记位名称
    const ATTR_CONTENT_SCORE = "contentScore";

    // DOM parsing classes currently only supports UTF-8 encoding -- DOM 解析类目前只支持 UTF-8 编码
    const DOM_DEFAULT_CHARSET = "utf-8";

    // Error Handling - 当判定失败时显示的内容
    const MESSAGE_CAN_NOT_GET = "Readability was unable to parse this page for content.";

    // DOM (analytical category) 解析类（PHP5 it is built 已内置）
    protected $DOM = null;

    // To parse the source code -- 需要解析的源代码
    protected $source = "";

    // Parent element lists in the section -- 章节的父元素列表
    private $parentNodes = array();

    // discardable tags - 需要删除的标签
    ...
    // Junk attributes -- 需要删除的属性
    private $junkAttrs = Array("style", "class", "onclick", "onmouseover", "align", "border", "margin");


    /**
     *      @param $input_char Encoded String, default utf-8 字符串的编码。默认 utf-8，可以省略
     */
    function __construct($source, $input_char = "utf-8") {
        $this->source = $source;

        // DOM parsing classes can handle UTF-8 character format (DOM 解析类只能处理 UTF-8 格式的字符)
    ...
    private function preparSource($string) {
        // Excluding the extra HTML coding marked to avoid parsing error (剔除多余的 HTML 编码标记，避免解析出错)
